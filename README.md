...
* **TLS 指纹混淆**：模拟 Chrome 等主流浏览器的 TLS 握手特征，绕过 Cloudflare 爬虫校验
* **嵌套解析M3U8**：支持循环解析Master.m3u8，自动识别BaseUrl、最高清晰度Video.m3u8
* **自动AES解密**：支持自动流式解密标准 AES-128 加密分片
* **断点续传**：流式下载，自动检测校验完整性，续传中断分片进度
* **多线程并发**：请求m3u8分片时多线程并发下载，榨干宽带
* **FFmpeg 自动封装**：分片下载完毕后自动调用 `FFmpeg`，无损快速合并
* **三模一体化运行**
